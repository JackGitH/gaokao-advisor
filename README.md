...
- **智能匹配**：输入分数或排名，按2026山东官方一分一段换算位次，自动推荐"冲一冲/稳一稳/保一保"三档院校
...
- **排名优先原则**：排名比分数更稳定，以位次为核心匹配依据；2026仅用于分数/位次换算，院校录取概率仍参考2023-2025历史录取数据
...
| GET /api/ranking-table?year=2026 | 一分一段表 |
| GET /api/subject-ranking-table?year=2026&subject=物理 | 选科一分一段表 |
...
- 2026：只使用山东省教育招生考试院公开的普通类分数线和一分一段表，用于分数/位次互转。
- 2023-2025：用于院校和专业历史录取位次匹配。
- 选科：导入山东省教育招生考试院《2024通用版普通高校拟在山东招生专业（类）选考科目要求》，适用于2025年和2026年考生。系统按“学校+专业”匹配官方选科要求，只过滤建议专业，不直接删除整所学校；缺失选科要求的数据不会被臆测删除。
